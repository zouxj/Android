 ...
 * 歌词定义
 * <p>
 * Created by zxj on 2017-1-6下午5:02:32
 * <p>
 */
public class LyricView extends TextView {
	...
	private int[] colors = { this.getResources().getColor(R.color.yelow), Color.WHITE };
	...
	private void initLyricInfo(AttributeSet attrs) {
		...
		mCurrentColor = ta.getColor(R.styleable.LrcView_lrcGradientStartColor, 0xffE2B051);
		mGradientEndColor = ta.getColor(R.styleable.LrcView_lrcGradientStartColor, 0xffffffff);
		mNormalColor = ta.getColor(R.styleable.LrcView_lrcNormalTextColor, 0xFFE2B051);
		// 初始化其他相关的值
...
//			StaticLayout myStaticLayout = new StaticLayout(text, paint, canvas.getWidth(), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
...
//			myStaticLayout.draw(canvas);
	...
	/** 初始化歌词 **/
	public void initLyricFile(InputStream stream) {
		// 解析歌词 保存到集合
		lyrics = LyricsParser.parserFromFile(stream);
		System.err.println(lyrics.get(0));
	}
}
